...
if len(weights) != num_criteria:
    print(f"Error: Number of weights must match number of criteria.")
    sys.exit(1)

if len(impacts) != num_criteria:
    print(f"Error: Number of impacts must match number of criteria.")
    sys.exit(1)

for impact in impacts:
    if impact not in ['+', '-']:
        print(f"Error: Impacts must be either '+' or '-'.")
        sys.exit(1)
